...
class DashboartdController extends Controller
{
    ...
    public function prosesResetPassword(Request $request){
        ...
        if(Hash::check($old_password, $user->password)){
            ...
            try {
                $user->save();
                return redirect(route('dashboard.resetPassword'))->with('pesan',['success','Selamat anda berhasil ubad password']);
            }catch (\Exception $e){
                return redirect(route('dashboard.resetPassword'))->with('pesan',['danger',' anda gagal ubad password']);
